...
def main():
    ...
    save_path = '/SAN/medic/IQT_ScoreMatching/Extrinsic_hallu_0.01_sam_0.1_interpolation_t200_multi_1.4entropy'
    ...
    #print(files[:5])
    files_new = []
    lst_files = os.listdir('/SAN/medic/IQT_ScoreMatching/HalluBench/final_full/dps_nohallu/')
    for f in files:
    ...
    for i, (ref_img, data_dict) in tqdm.tqdm(enumerate(data)):
        ...
        cond_img = np.load(f'/SAN/medic/IQT_ScoreMatching/HalluBench/final_full/dps_nohallu/{data_dict["file_id"][0]}/pred_{data_dict["slice_idx"][0]}_axial.npy')
        cond_img = th.tensor(cond_img, device=device).unsqueeze(0)    
        ...
        y_n = noiser(y)
        #print("IMAGE size: ", ref_img.shape)
        #y_n = y_n.clmap(min=0., max=2.)

        #Inject noise
        if configs['skip_timestep']:
            skip_x0 = y_n.clone().to(device) #ref_img.clone().to(device) #y_n.clone().to(device) #ref_img.clone().to(device) #torch.tensor(data).unsqueeze(0).unsqueeze(0).to(torch.float32).to(device)
        ...
        for j in range(num_hallu):
            ...
            while True:
                idx_lst, idx_size = select_patch(ref_img, size_min=16, size_max=17, bg_threshold=0.05) #24, 33
